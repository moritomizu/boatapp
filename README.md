...
7. Deploy後、Production URLを `https://boatapp.vercel.app` として確認します。

Vercelのプロジェクト名を `boatapp` にすると、利用可能であれば自動的に `boatapp.vercel.app` が割り当てられます。すでに同名URLが使われている場合は、VercelのProject Settings > Domainsから別名を設定するか、独自ドメインを追加してください。
...
```text
TaPiYoTa Grand Boat Club 確認URL: https://boatapp.vercel.app
...
Firebase接続へ進める場合は `.env.local` にFirebase Web Appの設定値を入れ、Repository層をFirestore実装に差し替えます。
...
Firebase接続時は以下へ変更し、Firestore Repository実装を追加してください。
